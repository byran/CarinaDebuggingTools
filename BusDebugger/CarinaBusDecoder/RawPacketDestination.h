...
public:
	...
	virtual void NonPacketBytesReceived(unsigned char* bytes,
										unsigned int length, uint32_t time) = 0;
	virtual void PacketReceived(BusHeader* header,
								unsigned int totalPacketLength,
								uint32_t time) = 0;
	virtual void PacketWithInvalidCrcReceived(BusHeader* header,
											  uint32_t time) = 0;
};
